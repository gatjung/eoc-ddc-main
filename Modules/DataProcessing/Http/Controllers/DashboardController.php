...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // graph 1 เพศ   
        $query_graph1 = DB::table('users');
        if($request->organization_type){
          $query_graph1->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph1->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph1->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph1->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }

        $query1 = $query_graph1   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    // ->select('users.organization', 'users.organization_type', 'users.position')         
        ...
        // graph 2 กลุ่มงาน          
        $query_graph2 = DB::table('users');
        if($request->organization_type){
          $query_graph2->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph2->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph2->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph2->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        ...
        // graph 3 ตำแหน่ง
        $query_graph3 = DB::table('users');
        if($request->organization_type){
          $query_graph3->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph3->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph3->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph3->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query3 = $query_graph3   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph5 = DB::table('users');
        if($request->organization_type){
          $query_graph5->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph5->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph5->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph5->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query5 = $query_graph5   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph6 = DB::table('users');
        if($request->organization_type){
          $query_graph6->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph6->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph6->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph6->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query6 = $query_graph6   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph7 = DB::table('users');
        if($request->organization_type){
          $query_graph7->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph7->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph7->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph7->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query7 = $query_graph7   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph8 = DB::table('users');
        if($request->organization_type){
          $query_graph8->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph8->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph8->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph8->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query8 = $query_graph8   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph9 = DB::table('users');
        if($request->organization_type){
          $query_graph9->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph9->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph9->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph9->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query9 = $query_graph9   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph10 = DB::table('users');
        if($request->organization_type){
          $query_graph10->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph10->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph10->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph10->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query10 = $query_graph10   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph11 = DB::table('users');
        if($request->organization_type){
          $query_graph11->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph11->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph11->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph11->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query11 = $query_graph11   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph12 = DB::table('users');
        if($request->organization_type){
          $query_graph12->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph12->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph12->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph12->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query12 = $query_graph12   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')  
        ...
        $query_graph13 = DB::table('users');
        if($request->organization_type){
          $query_graph13->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph13->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph13->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph13->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query13 = $query_graph13   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
        ...
        $query_graph14 = DB::table('users');
        if($request->organization_type){
          $query_graph14->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph14->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph14->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph14->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query14 = $query_graph14   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')        
        ...
        $query_graph15 = DB::table('users');
        if($request->organization_type){
          $query_graph15->where('organization_type', $request->organization_type);
        }
        if($request->organization){
          $query_graph15->where('organization', $request->organization);
        }
        if($request->roles){
          $query_graph15->where('roles', $request->roles);
        }
        if($request->start_date AND $request->start_date){
          $query_graph15->whereRaw('DATE_FORMAT(users.created_at, "%M %d %Y") >=DATE_FORMAT("'.$request->start_date.'", "%M %d %Y") and DATE_FORMAT(users.created_at, "%M %d %Y") <=DATE_FORMAT("'.$request->end_date.'", "%M %d %Y")');
        }
        $query15 = $query_graph15   
                    ->join('model_has_roles', 'users.id', '=', 'model_has_roles.model_id') 
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id') 
