...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
      <h2><span class="text-danger">แก้ไข</span> คำสั่งการ</h2>
      </div>
    </div>
  </div>
</section>
...
<section class="content">
  ...
  <form id="mem_form" action="{{Route('meeting.update',['id'=>$meeting->id])}}" method="post">
  @csrf
  ...
  <div class="card-body p-2">
    <span>เรื่อง</span><br>
    <textarea id="title" name="title" class="txtn form-control" placeholder="ชื่อการประชุม" require></textarea>
  </div>
  ...
  <div class="col-md-3">
      <div class="card-body p-2">
        ...
        <div class="input-group">
          <input type="number" class="form-control" name="at_num" min='1' value="{{$meeting->at_num}}" required>
          <div class="input-group-prepend">
            <span class="input-group-text">/</span>
          </div>
          <input type="number" class="form-control" name="at_year" value="{{$meeting->at_year}}"
    ...
    <div class="col-md-3">
      <div class="card-body p-2">
        <span>วันที่</span><br>
        <input name="at_date" id="at_date" class="datepicker form-control" require
    ...
    <div class="col-md-3">
      <div class="card-body p-2">
        ...
        <input name="at_start" id="at_start" class="timepicker form-control" require autocomplete="off"
    ...
    <div class="col-md-3">
      <div class="card-body p-2">
        <span>เวลาจบ</span><br>
        <input name="at_end" id="at_end" class="timepicker form-control" require autocomplete="off"
          type="text" placeholder="เริ่มประชุม" readonly="readonly" />
      </div>
    </div>
  ...
  <div class="card-body p-2">
        ...
        <input type="text" class="form-control" name="at_area" placeholder="ชื่อห้องประชุม"
          value="{{$meeting->at_area}}" required>
    </div>

    <div class="card-body p-2">
        <span>ประธาน</span><br>
        <input type="hidden" name="president_id" value="0">
        <input type="text" class="form-control" name="president_name" placeholder="ชื่อประธาน"
          value="{{$meeting->name_president}}" required>
    </div>
        ...
        <span>เนื้อหาการประชุม</span><br>
          ...
          <div class="mb-3">
            <textarea class="textarea" id="detail" name="detail"
                      style="width: 100%; height: 400px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{$meeting->detail}}</textarea>
          </div>


          <div class="card-body p-2">
            <span>สรุปประเด็นข้อสั่งการ</span><br>
            <input type="text" class="form-control" name="name_sum" 
            value="{{$meeting->name_sum}}" placeholder="ชื่อผู้สรุปประเด็นข้อสั่งการ">
          </div>
          <div class="card-body p-2">
            <span>ตรวจประเด็นข้อสั่งการ</span><br>
            <input type="text" class="form-control" name="name_check" 
            value="{{$meeting->name_check}}" placeholder="ชื่อผู้ตรวจประเด็นข้อสั่งการ">
          </div>
    ...
    <div class="callout callout-info d-none">
      ...
      <div class="card-body p-2">
      <input type="text" class="form-control" name="link_file" value="{{$meeting->link_file}}" placeholder="ลิงค์เอกสารแนบ">
      </div>
    </div>
    ...
    <div class="callout callout-info">
      ...
      <div class="card-body p-2 row">
        ...
        <div class="card-body p-2">
          <span>วันที่</span><br>
          <input name="book_at_date" id="book_at_date" class="datepicker form-control" require
            type="text" placeholder="วัน/เดือน/ปี" readonly="readonly" />
        </div>
      </div>
      ...
      <div class="col-md-3">
        <div class="card-body p-2">
          <span>เวลาเริ่ม</span><br>
          <input name="book_at_start" id="book_at_start" class="timepicker form-control" require autocomplete="off"
            type="text" placeholder="เริ่มประชุม" readonly="readonly" />
        </div>
      </div>
      ...
      <button type="button" class="btn btn-info btn-lg btn-block shadow-sm" onclick="pe_save()">
        <span class="far fa-save"></span> บันทึกการแก้ไข
      </button>
      </div><br>

  </div>
 
  <input type="hidden" name="del_orderid" id="del_orderid" value="">
  </form>
</div></section>
...
<script>
  ...
  $(function () {
    ...
    //-------------------------------
    $("#title").summernote("code", "{!!$meeting->title_tag!!}");

    var date = new Date("{{$meeting->at_date}} {{$meeting->at_start}}");
    var picker = $('#at_date').pickadate('picker');
    picker.set('select', date);

    var picker = $('#at_start').pickatime('picker');
    picker.set('select', date);

    var date = new Date("{{$meeting->at_date}} {{$meeting->at_end}}");
    var picker = $('#at_end').pickatime('picker');
    picker.set('select', date);


    var date = new Date("{{$meeting->book_at_date}} {{$meeting->book_at_start}}");
    var picker = $('#book_at_date').pickadate('picker');
    picker.set('select', date);

    var picker = $('#book_at_start').pickatime('picker');
    picker.set('select', date);
    //-------------------------------
    var order = {!! $order !!};
    var arrname = {!! $arrname !!};
    ...
    order.forEach(function(value) {
      var temp = JSON.parse(value['encode']);
      var central = [];
      var odpc = [];
      var other = '';
      var name = [];
      if(temp['central']) {
        central = temp['central'].split(",");
        central.forEach(function(value) {
          name.push(arrname['central'][value]);
        });
      }

      if(temp['odpc']) {
        odpc = temp['odpc'].split(",");
        odpc.forEach(function(value) {
          name.push(arrname['odpc'][value]);
        });
      }

      if(temp['other']) {
        other = temp['other'];
        name.push(temp['other']);
      }
...
function keep_data() {
  ...
  add_node(0, command, end_at, arr_name, central, odpc, other)
  area_reset();
}
//--------------------------
...
var curr_num = 0;
function add_node(order_id, command, end_at, name, central, odpc, other) {
  if(command != "") {
  curr_num++;
  ...
  var data ="<div class='col-md-5'>"
            ...
            +"<div class='row'>"
              ...
              +"<div class='col-11'>"
                +"<input type='hidden' value='"+order_id+"' name='data_id[]' id='DataID"+curr_num+"' />"
...
function fn_modal_del() {
  var node = "#DataID"+$("#del_curr_id").val();
  arr_del.push($(node).val());
  $("#del_orderid").val(arr_del);
