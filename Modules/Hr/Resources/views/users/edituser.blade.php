...
@section('content')
	<section class="content-header">
		...
		<section class="content">
			<div class="container-fluid">
				<form method="post" action="{{ route('users.update_hr', [$users->id]) }}">
					@csrf
					<div class="row">
						<div class="col-12">
							<div class="card">
								<div class="card-header bg-gradient-green">
									<h3 class="card-title">ข้อมูลส่วนตัวของ{{ $pre_th_arr[$users->prefix_th] }}{{ $users->name_th }} {{ $users->lname_th }}</h3>
											...
											<div class="form-group">
												...
												<select id="emp_level" class="form-control custom-select"  name="emp_level" data-live-search="true" required>
		                                            ...
		                                            @foreach($emp_level as $key_emp_level => $val_emp_level)
		                                                <option value="{{ $key_emp_level }}" {{ $key_emp_level == $users->emp_level ? 'selected' : '' }}>
		                                                	{{ $val_emp_level }}
                                                        </option>
		                                            @endforeach
                                                    @if ($users->emp_level == 3)
                                                        <option value="3" {{ $users->emp_level == 3 ? 'selected' : '' }}>
                                                            หัวหน้ากล่องภารกิจ
                                                        </option>
                                                    @endif
		                                        </select>
										...
										<div class="col-xs-12 col-sm-12 col-md-12">
											<div class="form-group">
												...
												@foreach($command_list as $key => $value)
													...
													<select class="command-single" name="comm_num[]">
                                    				...
                                    				<div class="input-group date" id="datepicker2">
				                                        <input class=" form-control input-medium" name="comm_date[]" type="text" data-provide="datepicker" data-date-language="th" data-date-format="yyyy-mm-dd" placeholder="ปี/เดือน/วัน" autocomplete="off" id="comm_date" value="{{ $value->comm_date }}">
				                                        <div class="input-group-append">
				                                            <div class="input-group-text"><i class="fa fa-calendar"></i></div>
				                                        </div>
				                                    </div>
				                                    ...
				                                    <button style="margin-top: 10px;" class="btn btn-success">
				                                    	<a href="{{ route('commandlist.DownloadFile',[$value->file_name, $value->id, $value->file_ori_name]) }}" type="button" style="color: white;"> ดาวน์โหลดไฟล์คำสั่ง </a>
				                                    </button>
				                                    @else
				                                    ...
				                                    <button style="margin-top: 10px;" class="btn btn-danger" type="button">
				                                    	<a href="{{ route('users.destroy.command_hr',$value->id)}}" style="color: white;">
												...
												<button type="button"  class="btn btn-success" style="margin-top: 10px; margin-bottom: 10px; width: 110px;" >
													<a href="{{ route('users.add_command_hr', [$users->id]) }}" style="color: white;">
														เพิ่มเลขคำสั่ง
													</a>
												</button>
								...
								<div class="card-body">
									<div class="row">
											...
											<div class="form-group">
												...
												<select class="form-control custom-select" name="talent_course">
			                                        ...
			                                        @foreach($talent_course as $value)
			                                            <option value="{{ $value->course_id }}" {{ $users->talent_course == $value->id ? 'selected' : '' }}>
			                                            	{{ $value->course }}
			                                            </option>
			                                        @endforeach
			                                    </select>
										...
										<div class="col-xs-12 col-sm-12 col-md-12">
                                            ...
                                            <a href="{{ route('users.chkcommanduser', $users->id) }}" class="btn btn-danger btn-lg">ไม่มีชื่อในกล่องภาระกิจ</a>
