...
<section class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
            @if(Route::current()->getName() == 'task_pending') <div class="card card-warning card-outline shadow">
            @elseif(Route::current()->getName() == 'task_success') <div class="card card-success card-outline shadow">
            @elseif(Route::current()->getName() == 'task_overdue') <div class="card card-danger card-outline shadow">
            @elseif(Route::current()->getName() == 'task_index') <div class="card card-primary card-outline shadow">
            @endif
                            ...
                            <div class="tab-pane fade active show" id="tabs-job" role="tabpanel" aria-labelledby="tabs-job-tab">
                                <div class=" table-responsive">
                                    <table class="table table-striped table-sm table-bordered table-hover" id="DTTH">
                                        @if(Route::current()->getName() == 'task_pending') <thead class="text-nowrap bg-gradient-warning">
                                        @elseif(Route::current()->getName() == 'task_success') <thead class="text-nowrap bg-gradient-success">
                                        @elseif(Route::current()->getName() == 'task_overdue') <thead class="text-nowrap bg-gradient-danger">
                                        @endif
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-nowrap text-right">
                                                    ...
                                                    <!-- <a href="{{ route('meeting.show',['id'=>$val->meet_id]) }}" class="btn bg-gradient-info btn-sm rounded-pill" target="_blank">คำสั่งการ</a> -->

                                                    <a href="{{ route('task_job',[
                            'meet_id'=>$val->meet_id,
                            'order_id'=>$val->order_id,
                            'roles_id'=>$val->roles_id
                          ]) }}" class="btn bg-gradient-pink btn-sm rounded-pill">รายละเอียด</a>
                                                </td>
                                            </tr>
                                            <?php $i++; ?>
                                            @endforeach
